...
function comboLegend(layer: number, pos?: number) {
  ...
  return `<div class="layer-combos"><h3>Combos${pos === undefined ? '' : ' avec cette touche'}</h3><p>Presse les touches ensemble pour déclencher l’action. Survole un combo pour repérer ses touches. Le délai indique le temps maximal entre les appuis.</p><div class="combo-list">${combos.map(c => `<button class="combo" data-combo="${escape(c.name)}" data-combo-layer="${layer}">${c.positions.map(p => {
    const raw = data.layers[layer].keys[p]
    return `<kbd>${escape(describe(raw === '&trans' ? data.layers[0].keys[p] : raw).label)}</kbd>`
  }).join(' + ')} <span>→ ${escape(describe(c.binding).label)}</span><small> · ${c.timeout} ms</small></button>`).join('')}</div></div>`
}
function detail() {
  if (!selected)
